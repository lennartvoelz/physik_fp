...
fig.set_xlabel(r'Resonatorlänge $L$ / $\mathrm{cm}$')
fig.set_ylabel(r'Leistung $P$ / $\mathrm{mW}$')
fig.grid()
ax.legend()
...
fig.set_xlabel(r'Polarisationswinkel $\Phi$ $^\circ$')
fig.set_ylabel(r'Leistung $P$ / $\mathrm{mW}$')
fig.grid()
ax.legend()
...
fig.set_xlabel(r'Abstand von der Strahlachse $x$ / $\mathrm{mm}$')
fig.set_ylabel(r'Leistung $P$ / $\mathrm{mW}$')
fig.grid()
ax.legend()
...
fig.set_xlabel(r'Resonatorlänge $L$ / $\mathrm{m}$')
fig.set_ylabel(r'Berechnete Resonatorlänge $L_\text{calc}$ / $\mathrm{m}$')
fig.grid()
...
lit_wel = 633
def abw(wel, lit):
    return (wel - lit) / lit * 100

print(f'Abweichung der berechneten Wellenlänge von der Literaturwellenlänge: {abw(W(dist_1, 1/g1, d1), lit_wel)} %')
print(f'Abweichung der berechneten Wellenlänge von der Literaturwellenlänge: {abw(W(dist_2, 1/g2, d2), lit_wel)} %')
print(f'Abweichung der berechneten Wellenlänge von der Literaturwellenlänge: {abw(W(dist_3, 1/g3, d3), lit_wel)} %')
print(f'Abweichung der berechneten Wellenlänge von der Literaturwellenlänge: {abw(W(dist_4, 1/g4, d4), lit_wel)} %')

print(abw(ufloat(0.956, 0.003), 1.0))
